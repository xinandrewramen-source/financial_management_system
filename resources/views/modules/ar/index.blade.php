...
@section('content')
    ...
    <div class="bg-white p-6 rounded-xl shadow" style="border-left:4px solid #F97316;">
        ...
        <p class="text-3xl font-black text-gray-900 mt-1">₱{{ number_format($totalReceivables, 2) }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-5 rounded-xl shadow border">
            <h2 class="text-md font-bold mb-3 text-gray-800">👤 Add New Payer</h2>
            <form action="/payers" method="POST" class="space-y-3">
                ...
                <button type="submit" class="w-full text-white text-sm font-bold py-2 rounded transition-colors" style="background-color:#F97316;" onmouseover="this.style.backgroundColor='#EA580C'" onmouseout="this.style.backgroundColor='#F97316'">Save Payer</button>
            </form>
        </div>

        <!-- Create Invoice Form -->
        <div class="bg-white p-5 rounded-xl shadow border">
            <h2 class="text-md font-bold mb-3 text-gray-800">📄 Create AR Invoice</h2>
            <form action="/invoices" method="POST" class="space-y-3">
                ...
                <button type="submit" class="w-full text-white text-sm font-bold py-2 rounded transition-colors" style="background-color:#F97316;" onmouseover="this.style.backgroundColor='#EA580C'" onmouseout="this.style.backgroundColor='#F97316'">Generate Invoice</button>
            </form>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($invoices as $inv)
                        <tr class="border-b hover:bg-slate-50">
                            ...
                            <td class="p-3 font-bold text-gray-900">₱{{ number_format($inv->total_amount, 2) }}</td>
                            <td class="p-3 text-gray-900 font-bold">₱{{ number_format($inv->paid_amount, 2) }}</td>
                            <td class="p-3 text-gray-900 font-bold">₱{{ number_format($inv->total_amount - $inv->paid_amount, 2) }}</td>
                            <td class="p-3">
